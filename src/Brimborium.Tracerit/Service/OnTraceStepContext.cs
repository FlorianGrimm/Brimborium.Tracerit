...
public struct OnTraceStepCurrentContext {
    ...
    public bool TryGetState<TState>([MaybeNullWhen(false)] out TState state)
        where TState : ValidatorExpressionState {
        return this._ExecutionState.TryGetState(this._Identifier, out state);
    }

    public bool TryGetState<TState>(int childIndex, [MaybeNullWhen(false)] out TState state)
        where TState : ValidatorExpressionState {
        return this._ExecutionState.TryGetState(this._Identifier.GetChildIdentifier(childIndex), out state);
    }

    public OnTraceStepCurrentContext GetChildContext(int childIndex) {
    ...
    public void SetStateSuccessfull(IValidatorExpression validatorExpression, ValidatorExpressionState state) {
