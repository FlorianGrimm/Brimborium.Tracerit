...
namespace Brimborium.Tracerit.Logger.Pool;

public class LoggerTracorDataSharedPool {    
    public const int DefaultMaxPoolSize = 2048;

    public static LoggerTracorDataSharedPool Current = new(DefaultMaxPoolSize);

    public LoggerTracorDataSharedPool() {
        var p=new ObjectPoolProvider();

        Microsoft.Extensions.ObjectPool.ObjectPool< LoggerTracorData> x=new 
    }

    //public readonly int Capacity;
    //private readonly LoggerTracorData?[] pool;
    //private long rentIndex;
    //private long returnIndex;

    //private LoggerTracorDataSharedPool(int capacity) {
    ...
    //            var loggerTracorData = Interlocked.Exchange(ref this.pool[rentSnapshot % this.Capacity], null);
    //            if (loggerTracorData == null && !this.TryRentCoreRare(rentSnapshot, out loggerTracorData)) {
    //                continue;
    //            }

    //            Debug.Assert(loggerTracorData.Source == LogRecord.LogRecordSource.FromSharedPool, "logRecord.Source was not FromSharedPool");
    //            loggerTracorData.ResetReferenceCount();
    //            return loggerTracorData;
    //        }
    //    }

    //    var newLogRecord = new LoggerTracorData() {
