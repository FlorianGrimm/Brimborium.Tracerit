...
namespace Brimborium.Tracerit.TestSupport.TestUtility;

public abstract class WebAppIntegrationBase : TUnit.Core.Interfaces.IAsyncInitializer, IAsyncDisposable {
    public abstract string GetForTestingCsproj();
    public abstract string GetProjectSourceCodeFolder();
    ...
    public Task RunServerToTest() {
        ...
        string pathToCsproj = this.GetForTestingCsproj();
    ...
    public async Task RunServerForReceiving() {
        await this.RunServerForReceiving((builder) => {
            var projectSourceCodeFolder = this.GetProjectSourceCodeFolder();
