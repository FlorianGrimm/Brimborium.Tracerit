param(
  ...
  [Parameter()][string]$targetFramework="net9.0",
...
$artifactsPath = Join-Path $rootDirectory "artifacts/bin/$testProjectName/$($configuration.ToLowerInvariant())"

Write-Host "Generate Coyote rewriting options JSON file."
$assemblies = Get-ChildItem $artifactsPath -Filter *.dll | Where-Object {$_.Name.StartsWith('Brimborium.Tracerit.')} | ForEach-Object {$_.Name}
...
dotnet exec "$artifactsPath/$testProjectName.dll" --framework $targetFramework --filter CategoryName=$categoryName
