...
namespace Brimborium.Tracerit.Collector.OOP.Test.TestUtility;

public sealed class WebAppIntegration : WebAppIntegrationBase {
    public override string GetForTestingCsproj() {
        var pathToCsproj = global::Brimborium.Tracerit.Collector.ForTesting.Program.GetBrimboriumTraceritCollectorForTestingCsproj();
        return pathToCsproj;
    }

    private static string? _GetProjectSourceCodeFolder;
    public override string GetProjectSourceCodeFolder() {
        return _GetProjectSourceCodeFolder ??= Intern();

        static string Intern([CallerFilePath] string value = "") { 
            var result = (System.IO.Path.GetDirectoryName(value) ?? throw new Exception());
            result = (System.IO.Path.GetDirectoryName(result) ?? throw new Exception());
            return result;
        }
    }
}
